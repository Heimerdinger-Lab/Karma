...
class read_quorum_reply : public task {
   public:
    ...
    std::shared_ptr<transport::frame> gen_frame() override {
        auto ret_frame =
            std::make_shared<transport::frame>(karma_rpc::OperationCode::OperationCode_READ_QUORUM);
        // header
...
class read_quorum_request : public task {
   public:
    ...
    static std::shared_ptr<read_quorum_request> from_frame(
        ...
        raft::read_quorum request{
            ...
            .id = static_cast<raft::read_id>(header->id())};
        return std::make_shared<read_quorum_request>(header->from_id(), header->group_id(),
                                                     request);
    };
    co_context::task<void> callback(std::shared_ptr<transport::frame> reply_frame) override {}
