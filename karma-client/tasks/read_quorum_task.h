...
public:
  ...
  std::shared_ptr<transport::frame> gen_frame() override {
    auto ret_frame = std::make_shared<transport::frame>(
        karma_rpc::OperationCode::OperationCode_READ_QUORUM);
    // header
  ...
  static std::shared_ptr<read_quorum_reply> from_frame(std::shared_ptr<transport::frame> frame) {
      ...
      raft::read_quorum_reply reply {.current_term =  static_cast<raft::term_t>(header->current_term()), .commit_idx = static_cast<raft::index_t>(header->commit_idx()), .id =  static_cast<raft::read_id>(header->id())};
      return std::make_shared<read_quorum_reply>(header->from_id(), header->group_id(), reply);
  };
...
public:
  ...
  std::shared_ptr<transport::frame> gen_frame() override {
    ...
    flatbuffers::FlatBufferBuilder header_builder;
    auto header =
        karma_rpc::CreateReadQuorum(header_builder, m_from_id, m_group_id,
                                    m_request.current_term, m_request.leader_commit_idx, m_request.id);
    header_builder.Finish(header);
    // set the frame
    uint8_t *buffer = header_builder.GetBufferPointer();
    int size = header_builder.GetSize();
  ...
  static std::shared_ptr<read_quorum_request> from_frame(std::shared_ptr<transport::frame> frame) {
      ...
      raft::read_quorum request{.current_term = static_cast<raft::term_t>(header->current_term()), .leader_commit_idx = static_cast<raft::index_t>(header->leader_commit_idx()), .id = static_cast<raft::read_id>(header->id())};
      return std::make_shared<read_quorum_request>(header->from_id(), header->group_id(), request);
  };
