...
public:
  ...
  std::shared_ptr<transport::frame> gen_frame() override {
    auto ret_frame = std::make_shared<transport::frame>(
        karma_rpc::OperationCode::OperationCode_TIME_OUT);
    // header
    flatbuffers::FlatBufferBuilder header_builder;
    auto header = karma_rpc::CreateTimeOut(header_builder, m_from_id,
                                           m_group_id, m_request.current_term);
    ...
    static std::shared_ptr<time_out_request> from_frame(std::shared_ptr<transport::frame> frame) {
        ...
        return std::make_shared<time_out_request>(header->from_id(), header->group_id(), request);
    };
