...
std::unique_ptr<client::append_entry_request> client::append_entry_request::from_frame(
    ...
    // TODO use the flatbuffers::Verifier
    ...
    for (int i = 0; i < log_entries->size(); ++i) {
        ...
        entry->idx = log_entry->index();
        entry->term = log_entry->term();
        if (log_entry->command()->size() == 0) {
            entry->data = raft::log_entry::dummy{};
        } else {
            entry->data = log_entry->command()->str();
        }
    ...
    raft::append_request request{
        ...
        .entries = entries};
    BOOST_LOG_TRIVIAL(trace) << "Generate an append entry request from frame";
...
std::unique_ptr<client::append_entry_reply> client::append_entry_reply::from_frame(
    ...
    BOOST_LOG_TRIVIAL(trace) << "Generate an append entry reply from frame";
