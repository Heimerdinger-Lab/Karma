...
public:
  vote_request(uint64_t m_from_id, uint64_t m_group_id,
  ...
  std::shared_ptr<transport::frame> gen_frame() override {
    auto ret_frame = std::make_shared<transport::frame>(
        karma_rpc::OperationCode::OperationCode_VOTE);
    // header
    flatbuffers::FlatBufferBuilder header_builder;
    auto header = karma_rpc::CreateVoteRequest(
        header_builder, m_from_id, m_group_id, m_request.current_term,
    ...
    static std::shared_ptr<vote_request> from_frame(std::shared_ptr<transport::frame> frame) {
        ...
        raft::vote_request request {.current_term = static_cast<raft::term_t>(header->current_term()), .last_log_idx = static_cast<raft::index_t>(header->last_log_idx()), .last_log_term =  static_cast<raft::term_t>(header->last_log_term()), .is_prevote = false, .force = false};
        return std::make_shared<vote_request>(header->from_id(), header->group_id(), request);
    };
...
public:
  ...
  std::shared_ptr<transport::frame> gen_frame() override {
    ...
    // header
    flatbuffers::FlatBufferBuilder header_builder;
    auto header = karma_rpc::CreateVoteReply(header_builder, m_from_id,
                                               m_group_id, m_reply.current_term,
                                               m_reply.vote_granted);
    header_builder.Finish(header);
    // set the frame
    uint8_t *buffer = header_builder.GetBufferPointer();
    int size = header_builder.GetSize();
    ...
    static std::shared_ptr<vote_reply> from_frame(std::shared_ptr<transport::frame> frame) {
        ...
        raft::vote_reply reply{.current_term = static_cast<raft::term_t>(header->current_term()), .vote_granted = static_cast<bool>(header->vote_granted()), .is_prevote = false};
        return std::make_shared<vote_reply>(header->from_id(), header->group_id(), reply);
    };
  co_context::task<void>
  callback(std::shared_ptr<transport::frame> reply_frame) override {}
private:
