...
public:
    ...
    std::shared_ptr<transport::frame> gen_frame() override {
        auto ret_frame = std::make_shared<transport::frame>(karma_rpc::OperationCode::OperationCode_ECHO);
        // header
    ...
    static std::shared_ptr<echo_reply> from_frame(std::shared_ptr<transport::frame> frame) {
        ...
        return std::make_shared<echo_reply>(header->from_id(), header->group_id(), header->msg()->str());
    };
    std::string msg() {
        return m_msg;
    }
...
public:
    echo_request(uint64_t from_id, uint64_t group_id, std::string msg, std::shared_ptr<co_context::channel<std::shared_ptr<echo_reply>>> prom = {}) 
        : m_from_id(from_id)
        , m_group_id(group_id)
        , m_msg(msg) 
        , m_prom(prom) {}
    ...
    static std::shared_ptr<echo_request> from_frame(std::shared_ptr<transport::frame> frame) {
        ...
        std::cout << "from_frame, header: " << header->msg()->str() << std::endl;
        return std::make_shared<echo_request>(header->from_id(), header->group_id(), header->msg()->str());
