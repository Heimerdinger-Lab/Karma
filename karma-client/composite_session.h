...
class composite_session {
   public:
    static co_context::task<std::optional<std::shared_ptr<composite_session>>>
    new_composite_session(std::string host, uint16_t port) {
        ...
        if (co_context::inet_address::resolve(host, port, addr)) {
            ...
            auto sock =
                std::make_shared<co_context::socket>(co_context::socket::create_tcp(addr.family()));
            // 连接一个 server
    ...
    composite_session(std::shared_ptr<session> sess) : m_session(sess){};
   ...
   private:
    ...
    std::shared_ptr<session> pick_session() { return m_session; }
};
};  // namespace client