...
log_entry_ptr& log::get_entry(index_t i) { return _log[i - _first_idx]; }

const log_entry_ptr& log::get_entry(index_t i) const {
  return _log[i - _first_idx];
}

size_t log::range_memory_usage(log_entries::iterator first,
                               log_entries::iterator last) const {
  size_t result = 0;
...
bool log::empty() const { return _log.empty(); }
...
index_t log::next_idx() const { return last_idx() + index_t(1); }
...
void log::init_last_conf_idx() {
  for (auto it = _log.rbegin();
       it != _log.rend() && (**it).idx != _snapshot.idx; ++it) {
...
std::pair<bool, term_t> log::match_term(index_t idx, term_t term) const {
  ...
  return my_term == term ? std::make_pair(true, term_t(0))
                         : std::make_pair(false, my_term);
}
...
const configuration& log::get_configuration() const {
  return _last_conf_idx
             ? std::get<configuration>(_log[_last_conf_idx - _first_idx]->data)
             : _snapshot.config;
}
...
index_t log::maybe_append(std::vector<log_entry_ptr>&& entries) {
  ...
  for (auto& e : entries) {
    if (e->idx <= last_idx()) {
      if (e->idx < _first_idx) {
        co_context::log::d(
            "append_entries: skipping entry with idx {} less than log start {}",
            e->idx, _first_idx);
        continue;
      }
      if (e->term == get_entry(e->idx)->term) {
        co_context::log::d(
            "append_entries: entries with index {} has matching terms {}",
            e->idx, e->term);
        continue;
      }
      co_context::log::d(
          "append_entries: entries with index {} has non matching terms "
          "e.term={}, _log[i].term = {}",
...
const configuration* log::get_prev_configuration() const {
  ...
  // _last_conf_idx <= _snapshot.idx means we only have the last configuration
  // (from the snapshot).
  return nullptr;
}

size_t log::apply_snapshot(snapshot_descriptor&& snp,
  ...
  if (idx > last_idx()) {
    ...
    for (size_t i = 0; i < max_trailing_entries && entries_to_remove > 0; ++i) {
      trailing_bytes +=
          memory_usage_of(*_log[entries_to_remove - 1], _max_command_size);
    ...
    released_memory =
        range_memory_usage(_log.begin(), _log.begin() + entries_to_remove);
    _log.erase(_log.begin(), _log.begin() + entries_to_remove);
    _log.shrink_to_fit();
    _memory_usage -= released_memory;
    _first_idx = _first_idx + index_t{static_cast<index_t>(entries_to_remove)};
  }
