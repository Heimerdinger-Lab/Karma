...
//     read_id id;
//     index_t idx;
//     std::shared_ptr<co_context::channel<read_barrier_reply>> promise;
...
//     co_context::task<add_entry_reply> execute_modify_config(server_id from,
//         std::vector<config_member> add, std::vector<server_id> del) override;
...
//     co_context::task<> tick() override;
//     raft::server_id id() const override;
//     void set_applier_queue_max_size(size_t queue_max_size) override;
//     co_context::task<> stepdown(logical_clock::duration timeout) override;
//     co_context::task<> modify_config(std::vector<config_member> add, std::vector<server_id> del) override;
...
//     co_context::channel<std::monostate> _stepdown_promise;
//     // std::optional<shared_promise<>> _leader_promise;
//     co_context::channel<std::monostate> _leader_promise;
//     awaited_conf_change _non_joint_conf_commit_promise;
//     // std::optional<shared_promise<>> _state_change_promise;
//     co_context::channel<std::monostate> _state_change_promise;
...
//     std::optional<std::string> _aborted;
...
//     co_context::mutex _applied_index_changed_mtx;
//     co_context::condition_variable _applied_index_changed;
...
//     co_context::channel<applier_fiber_message> _apply_entries;
//     struct stats {
...
//         std::shared_ptr<co_context::channel<std::monostate>> done;
...
//     //     std::shared_ptr<co_context::channel<std::monostate>> f;
//     //     // future<> f;
...
//     co_context::task<> notify_waiters(std::map<index_t, op_status>& waiters, const std::vector<log_entry_ptr>& entries);
...
//     // co_context::channel<std::monostate> _ch;
//     // co_context::task<> io_fiber_outer();
...
//     template <typename Message> co_context::task<> send_message(server_id id, Message m);
...
//     // co_context::task<> _applier_status = make_ready_future<>();
//     // co_context::task<> _io_status = make_ready_future<>();
...
//     co_context::task<> wait_for_leader();

//     co_context::task<> wait_for_state_change() override;
...
//     co_context::channel<std::monostate> _tick_promise;
//     co_context::task<> wait_for_next_tick();
...
//     // }
//     // co_context::task<> do_on_leader_with_retries(seastar::abort_source* as, AsyncAction&& action);
...
//     co_context::co_spawn(io_fiber(stable_idx));
//     // start fiber to apply committed entries
//     co_context::co_spawn(applier_fiber());
...
//         co_await _applied_index_changed.wait(_applied_index_changed_mtx);
//     }

//     co_return;
// }

// co_context::task<> server_impl::wait_for_next_tick() {
//     check_not_aborted();
//     co_await _tick_promise.acquire();
//     // if (!_tick_promise) {
...
// co_context::task<> server_impl::wait_for_leader() {
...
//     // }
//     co_await _leader_promise.acquire();
//     // try {
...
// co_context::task<> server_impl::wait_for_state_change() {
...
//     //     throw request_aborted();
//     // }
//     co_await _state_change_promise.acquire();
// }

// co_context::task<> server_impl::wait_for_entry(entry_id eid, wait_type type) {
...
//     auto [it, inserted] = container.emplace(eid.idx, op_status{eid.term, std::make_shared<co_context::channel<std::monostate>>()});
//     if (!inserted) {
...
//         container.erase(it);
//         std::tie(it, inserted) = container.emplace(eid.idx, op_status{eid.term, std::make_shared<co_context::channel<std::monostate>>()});
...
//             // co_await prev_wait.done->release(dropped_entry{});
//             _stats.waiters_awoken++;
//         } else {
...
//     co_await it->second.done->acquire();
//     // logger.trace("[{}] done waiting for {}.{}", id(), eid.term, eid.idx);
//     co_return;
// }

// co_context::task<entry_id> server_impl::add_entry_on_leader(command cmd) {
...
// co_context::task<add_entry_reply> server_impl::execute_add_entry(server_id from, command cmd) {
...
// co_context::task<> server_impl::add_entry(command command, wait_type type) {
...
//     }
//     auto leader = _fsm->current_leader();
//     add_entry_reply reply;
//     if (leader == _id) {
//         reply = co_await execute_add_entry(leader, command);
//     } else {
//         reply = co_await _rpc->send_add_entry(leader, command);
//     }

//     if (std::holds_alternative<raft::entry_id>(reply)) {
//         co_await wait_for_entry(std::get<raft::entry_id>(reply), type);
//         co_return;
//     }
...
// co_context::task<add_entry_reply> server_impl::execute_modify_config(server_id from,
//     std::vector<config_member> add, std::vector<server_id> del) {

// //     if (from != _id && !_fsm->get_configuration().contains(from)) {
// //         // Do not accept entries from servers removed from the
// //         // configuration.
// //         co_return add_entry_reply{not_a_member{fmt::format("Modify config from {} was discarded since "
// //                                                          "it is not part of the configuration", from)}};
// //     }
// //     try {
...
// co_context::task<> server_impl::modify_config(std::vector<config_member> add, std::vector<server_id> del) {
//     // if (!_config.enable_forwarding) {
...
//     // std::cout << "receive : requets_vote" << std::endl;
...
// co_context::task<> server_impl::notify_waiters(std::map<index_t, op_status>& waiters,
...
//         co_await it->second.promise->release();
//         it = _awaited_indexes.erase(it);
//     }
// }

// template <typename Message>
// co_context::task<> server_impl::send_message(server_id id, Message msg) {
...
//     return result;
// }
// // co_context::task<> server_impl::io_fiber_outer() {
// //     co_await _ch.acquire();
// // };
// co_context::task<> server_impl::io_fiber(index_t last_stable) {
...
//                 // load it.
//                 if (!is_local) {
//                     co_await _apply_entries.release(std::move(snp));
//                 }
//             }
...
//             }
//             // std::cout << "messages.size = " << batch.messages.size() << std::endl;
//              // After entries are persisted we can send messages.
//             for (auto&& m : batch.messages) {
//                 try {
//                     co_await send_message(m.first, std::move(m.second));
//                 } catch(...) {
...
//                             co_await _non_joint_conf_commit_promise.promise->release();
...
//                 co_await _apply_entries.release(std::move(batch.committed));
//             }
...
//             //         co_await _reads.front().promise->release(_reads.front().idx);
//             //         _reads.pop_front();
//             //     }
//             // }
//             if (!_fsm->is_leader()) {
...
//                 // }
//                 co_await _stepdown_promise.release();
...
//                     co_await _apply_entries.release(removed_from_config{});
//                 }
//                 // request aborts of snapshot transfers
//                 abort_snapshot_transfers();
//                 // abort all read barriers
//                 for (auto& r : _reads) {
//                     co_await r.promise->release(not_a_leader{_fsm->current_leader()});
...
//                 // }
//                 // _stepdown_promise.release(Args args...)
//             }
//             // if (_leader_promise && _fsm->current_leader()) {
//             //     std::exchange(_leader_promise, std::nullopt)->set_value();
//             // }
//             co_await _leader_promise.release();
//             co_await _state_change_promise.release();
//             std::cout << "_apply_entries = " << _apply_entries.size() << std::endl;
...
// co_context::task<snapshot_reply> server_impl::apply_snapshot(server_id from, install_snapshot snp) {
...
// co_context::task<> server_impl::wait_for_apply(index_t idx) {
//     // if (as && as->abort_requested()) {
//     //     throw request_aborted();
//     // }
...
//         //     assert(it->second.abort);
//         // }
//         co_await it->second.promise->acquire();
//     }
// }

// co_context::task<read_barrier_reply> server_impl::execute_read_barrier(server_id from) {
//     // check_not_aborted();
...
//             co_return read_barrier_reply{std::monostate{}};
//         }
//     } catch (not_a_leader& err) {
//         // return make_ready_future<read_barrier_reply>(err);
//         co_return read_barrier_reply{err};
//     }
...
//     // }
//     // auto promise = std::make_shared<co_context::channel<read_barrier_reply>>();
//     // _reads.push_back({rid->first, rid->second, promise});
...
//     // auto x =  co_await read->promise->acquire();
//     // auto x = co_await promise->acquire();
//     co_return rid->second;
// }

// co_context::task<read_barrier_reply> server_impl::get_read_idx(server_id leader) {
//     if (_id == leader) {
...
// co_context::task<> server_impl::read_barrier() {
//     // logger.trace("[{}] read_barrier start", _id);
//     std::cout << "read_barrier!" << std::endl;
//     index_t read_idx;
...
//     res = co_await get_read_idx(leader);
...
// co_context::task<> server_impl::abort(std::string reason) {
// //     _is_alive = false;
...
// co_context::task<> server_impl::set_configuration(config_member_set c_new) {
// //     check_not_aborted();
...
// co_context::task<> server_impl::wait_election_done() {
//     while (_fsm->is_candidate()) {
//         co_await co_context::yield();
//     };
// }

// co_context::task<> server_impl::wait_log_idx_term(std::pair<index_t, term_t> idx_log) {
//     while (_fsm->log_last_term() < idx_log.second || _fsm->log_last_idx() < idx_log.first) {
//         co_await co_context::timeout(5us);
//     }
// }
...
// co_context::task<> server_impl::tick() {
...
// co_context::task<> server_impl::stepdown(logical_clock::duration timeout) {
// //     if (_stepdown_promise) {
