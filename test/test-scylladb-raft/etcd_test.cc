...
BOOST_AUTO_TEST_CASE(test_leader_election_overwrite_newer_logs) {
    ...
    raft::log_entry_ptr lep1 = std::make_shared<log_entry>(log_entry{term_t{1}, index_t{1}, raft::log_entry::dummy{}});
    ...
    raft::log_entry_ptr lep2 = std::make_shared<log_entry>(log_entry{term_t{1}, index_t{1}, raft::log_entry::dummy{}});
    ...
    raft::log_entry_ptr lep3 = std::make_shared<log_entry>(log_entry{term_t{2}, index_t{1}, raft::log_entry::dummy{}});
...
void handle_proposal(unsigned nodes, std::vector<int> accepting_int) {
    ...
    for (auto id: accepting_int) {
        accepting.insert(raft::server_id{static_cast<server_id>(id)});
    }

    std::vector<raft::server_id> ids;     // ids of leader 1 .. #nodes
    for (unsigned i = 1; i < nodes + 1; ++i) {
        ids.push_back(raft::server_id{i});
    }

    raft::configuration cfg = config_from_ids(ids);
    raft::log log1{raft::snapshot_descriptor{.config = cfg}};
    raft::fsm fsm1(raft::server_id{1}, term_t{}, server_id{}, std::move(log1),
    ...
    for (unsigned i = 2; i < nodes + 1; ++i) {
        fsm1.step(raft::server_id{i}, raft::vote_reply{output1.term_and_vote->first, true, false});
    }
    BOOST_CHECK(fsm1.is_leader());
    output1 = fsm1.get_output();
