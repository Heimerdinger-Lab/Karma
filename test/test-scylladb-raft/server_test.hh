...
struct cmd {
    ...
    std::string key;
    std::string value;
    size_t size() const { return key.size() + value.size() + 4; }
...
using network_message =
    std::variant<raft::append_request, raft::append_reply, raft::vote_request, raft::vote_reply,
                 raft::install_snapshot, raft::snapshot_reply, raft::timeout_now, raft::read_quorum,
                 raft::read_quorum_reply, execute_barrier_on_leader,
                 execute_barrier_on_leader_reply, add_entry_message, add_entry_reply_message>;

class impure_state_machine : public raft::state_machine {
   public:
    impure_state_machine(raft::server_id id) : _id(id) {}
    ...
    std::string get(std::string key) { return _values[key]; }
...
class rpc : public raft::rpc {
   public:
    rpc(raft::server_id id,
        std::function<void(raft::server_id from, raft::server_id to, network_message)> send)
    ...
    co_context::task<> receive(raft::server_id from, network_message msg) {
        std::cout << "id = " << _id << ", from = " << from << ", msg = " << msg.index()
                  << std::endl;
        if (std::holds_alternative<raft::append_request>(msg)) {
            ...
            co_await _read_barrier_channel->release(
                std::get<execute_barrier_on_leader_reply>(msg).reply);
        } else if (std::holds_alternative<add_entry_message>(msg)) {
            auto reply =
                co_await _client->execute_add_entry(from, std::get<add_entry_message>(msg).cmd);
            _send(_id, from, msg);
    ...
    void send_read_quorum_reply(raft::server_id id,
    ...
    void on_configuration_change(raft::server_address_set add, raft::server_address_set del) {}
...
class ps : public raft::persistence {
   public:
    ...
    co_context::task<raft::index_t> load_commit_idx() override { co_return m_index; }

    co_context::task<> store_log_entries(const std::vector<raft::log_entry_ptr>& entries) override {
        m_entries = entries;
        co_return;
    }
    co_context::task<raft::log_entries> load_log() override { co_return m_entries; }
    co_context::task<> truncate_log(raft::index_t idx) override { co_return; }
    co_context::task<> abort() override { co_return; }
    co_context::task<> store_snapshot_descriptor(const raft::snapshot_descriptor& snap,
...
class fd : public raft::failure_detector {
   public:
    ...
    bool is_alive(raft::server_id server) override { return true; }
};

class raft_server {
   public:
    raft_server(raft::server_id id, impure_state_machine& sm, rpc& rpc_,
                std::unique_ptr<raft::server>&& svr)
        : _id(id), _sm(sm), _rpc(rpc_), _server(std::move(svr)){};
    bool is_leader() { return _server->is_leader(); }
    static std::unique_ptr<raft_server> create(
        ...
        auto svr = raft::create_server(id, std::move(rpc_), std::move(sm), std::move(ps_),
                                       std::move(fd_), config);
    ...
    co_context::task<> start() { co_await _server->start(); }
...
class cluster {
   ...
   public:
    cluster(uint32_t count) {
        _count = count;
        _servers.push_back(
            raft_server::create(100, std::bind(&cluster::send, this, std::placeholders::_1,
                                               std::placeholders::_2, std::placeholders::_3)));
        for (int i = 1; i <= _count; i++) {
            _servers.push_back(
                raft_server::create(i, std::bind(&cluster::send, this, std::placeholders::_1,
                                                 std::placeholders::_2, std::placeholders::_3)));
        }
    }
    co_context::task<> tick() {
    ...
    co_context::task<> start() {
        ...
        // co_context::co_spawn([](std::shared_ptr<cluster> cls) ->
        // co_context::task<> {
        //     while(1) {
    ...
    void send(raft::server_id from, raft::server_id to, network_message msg) {
        // _queue.push()
        // auto delivery_time = _clock.now() +
        // raft::logical_clock::duration{_delivery_delay(_rnd)};
        uint32_t delivery_time = _time + getRandomNumber(0, 10);
        if (to == 0) return;
        // std::cout << "from: " << from << ", to: " << to << ", time: " <<
        // delivery_time << ", "  << std::endl;
        _event.push_back(task{.time = delivery_time, .from = from, .to = to, .msg = msg});
    }
    std::shared_ptr<raft_server> get_raft_server(int idx) { return _servers[idx]; }
