...
raft::server_id id() {
    static int id = 0;
    return raft::server_id{static_cast<raft::server_id>(++id)};
}
...
raft::server_id to_raft_id(size_t int_id) {
    return raft::server_id{int_id};
}

raft::server_address to_server_address(size_t int_id) {
    return server_addr_from_id(raft::server_id{int_id});
}
