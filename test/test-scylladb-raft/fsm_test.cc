...
BOOST_AUTO_TEST_CASE(test_snapshot_follower_is_quiet) {
    ...
    log.emplace_back(std::make_shared<raft::log_entry>(raft::log_entry{term_t{10}, index_t{1000}}));
...
BOOST_AUTO_TEST_CASE(test_confchange_a_to_b) {
    ...
    log.emplace_back(std::make_shared<raft::log_entry>(B.add_entry(config_from_ids({A_id}))));
    log.stable_to(log.last_idx());
