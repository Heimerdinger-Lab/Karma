...
//      .total_values                how many entries to append to leader nodes
//      (default 100) .initial_term                initial term # for setup
//      .initial_leader              what server is leader
//      .initial_states              initial logs of servers
//          .le                      log entries
//      .initial_snapshots           snapshots present at initial state for
//      servers .updates                     updates to execute on these servers
//          entries{x}               add the following x entries to the current
//          leader new_leader{x}            elect x as new leader
//          partition{a,b,c}         Only servers a,b,c are connected
//          partition{a,leader{b},c} Only servers a,b,c are connected, and make
//          b leader set_config{a,b,c}        Change configuration on leader
...
class hasher_int {
   ...
   public:
    static void set_commutative(bool commutative) { _commutative = commutative; }
...
struct initial_state {
    ...
    raft::server::configuration server_config =
        raft::server::configuration{.append_request_threshold = 200};
};
...
struct set_config_entry {
    ...
    set_config_entry(size_t idx, bool can_vote = true) : node_idx(idx), can_vote(can_vote) {}
};
...
struct check_rpc_config {
    ...
    check_rpc_config(std::vector<node_id> nodes, rpc_address_set addrs)
        : nodes(nodes), addrs(addrs) {}
};
...
struct check_rpc_added {
    ...
    check_rpc_added(std::vector<node_id> nodes, size_t expected)
...
struct check_rpc_removed {
    ...
    check_rpc_removed(std::vector<node_id> nodes, size_t expected)
        : nodes(nodes), expected(expected) {}
};
...
using update = std::variant<entries, new_leader, partition, isolate, disconnect, stop, reset,
                            wait_log, set_config, check_rpc_config, check_rpc_added,
...
using snapshots =
    std::unordered_map<raft::server_id, std::unordered_map<raft::snapshot_id, snapshot_value>>;
using persisted_snapshots =
    std::unordered_map<raft::server_id, std::pair<raft::snapshot_descriptor, snapshot_value>>;

extern seastar::semaphore snapshot_sync;
// application of a snapshot with that id will be delayed until snapshot_sync is
// signaled
extern raft::snapshot_id delay_apply_snapshot;
// sending of a snapshot with that id will be delayed until snapshot_sync is
// signaled
...
struct rpc_config {
    ...
    // How many nodes per server, rounded to closest power of 2 (fast prefix
    // check)
...
class raft_cluster {
    using apply_fn =
        std::function<size_t(raft::server_id id, const std::vector<raft::command_cref>& commands,
   ...
   public:
    raft_cluster(test_case test, apply_fn apply, size_t apply_entries, size_t first_val,
                 size_t first_leader, bool prevote, typename Clock::duration tick_delta,
    ...
    future<> reset_server(size_t id,
                          initial_state state);  // Reset a stopped server
...
class raft_cluster<Clock>::state_machine : public raft::state_machine {
   ...
   public:
    lw_shared_ptr<hasher_int> hasher;
    state_machine(raft::server_id id, apply_fn apply, size_t apply_entries, snapshots* snapshots)
        : _id(id),
    ...
    future<> apply(const std::vector<raft::command_cref> commands) override {
        ...
        if (n && _seen >= _apply_entries) {
            if (_seen > _apply_entries) {
                ...
                tlogger.warn("sm::apply[{}]: _seen ({}) overshot _apply_entries ({})", _id, _seen,
                             _apply_entries);
            }
            _done.set_value();
        }
    ...
    future<raft::snapshot_id> take_snapshot() override {
        ...
        tlogger.debug("sm[{}] takes snapshot id {} {} seen {}", _id,
                      (*_snapshots)[_id][snp_id].hasher.finalize_uint64(), snp_id, _seen);
        (*_snapshots)[_id][snp_id].idx = raft::index_t{_seen};
        return make_ready_future<raft::snapshot_id>(snp_id);
    }
    void drop_snapshot(raft::snapshot_id snp_id) override { (*_snapshots)[_id].erase(snp_id); }
    future<> load_snapshot(raft::snapshot_id snp_id) override {
        hasher = make_lw_shared<hasher_int>((*_snapshots)[_id][snp_id].hasher);
        tlogger.debug("sm[{}] loads snapshot {} idx={}", _id,
                      (*_snapshots)[_id][snp_id].hasher.finalize_uint64(),
                      (*_snapshots)[_id][snp_id].idx);
    ...
    future<> done() { return _done.get_future(); }
};
...
class raft_cluster<Clock>::persistence : public raft::persistence {
   ...
   public:
    ...
    future<> store_commit_idx(raft::index_t) override { co_return; }
    future<raft::index_t> load_commit_idx() override { co_return raft::index_t{0}; }
    future<> store_snapshot_descriptor(const raft::snapshot_descriptor& snap,
                                       size_t preserve_log_entries) override {
        (*_persisted_snapshots)[_id] = std::make_pair(snap, (*_snapshots)[_id][snap.id]);
        tlogger.debug("sm[{}] persists snapshot {}", _id,
                      (*_snapshots)[_id][snap.id].hasher.finalize_uint64());
        return make_ready_future<>();
    }
...
struct raft_cluster<Clock>::connected {
    struct connection {
        raft::server_id from;
        raft::server_id to;
        bool operator==(const connection& o) const { return from == o.from && to == o.to; }
    };
    ...
    void connect_all() { disconnected.clear(); }
...
class raft_cluster<Clock>::failure_detector : public raft::failure_detector {
   ...
   public:
    ...
    bool is_alive(raft::server_id server) override { return (*_connected)(server, _id); }
};
...
class raft_cluster<Clock>::rpc : public raft::rpc {
   ...
   public:
    rpc(raft::server_id id, connected* connected, snapshots* snapshots, rpc_net& net,
        rpc_config rpc_config)
        : _id(id),
    ...
    bool drop_packet() { return _rpc_config.drops && !(rand() % 5); }
    ...
    future<raft::snapshot_reply> send_snapshot(raft::server_id id,
                                               ...
                                               seastar::abort_source& as) override {
        if (!_net.count(id)) {
    ...
    future<> send_append_entries(raft::server_id id,
                                 const raft::append_request& append_request) override {
        if (!_net.count(id)) {
            return make_exception_future(
                std::runtime_error("trying to send a message to an unknown node"));
        }
        if (!(*_connected)(id, _id)) {
            return make_exception_future<>(
                std::runtime_error("cannot send append since nodes are disconnected"));
        }
        if (!drop_packet()) {
            if (_delays) {
                return with_gate(_gate, [&, this]() mutable -> future<> {
                    return seastar::sleep(get_delay(id) + rand_extra_delay())
                        .then(
        ...
        if (!drop_packet()) {
            if (_delays) {
                (void)with_gate(_gate, [&, this]() mutable -> future<> {
                    return seastar::sleep(get_delay(id) + rand_extra_delay())
                        .then([this, id = std::move(id), reply = std::move(reply)] {
                            if ((*_connected)(id, _id)) {
    ...
    void send_vote_request(raft::server_id id, const raft::vote_request& vote_request) override {
        ...
        if (_delays) {
            (void)with_gate(_gate, [&, this]() mutable -> future<> {
                return seastar::sleep(get_delay(id) + rand_extra_delay())
                    .then([this, id = std::move(id), vote_request = std::move(vote_request)] {
                        if ((*_connected)(id, _id)) {
    ...
    future<raft::read_barrier_reply> execute_read_barrier_on_leader(raft::server_id id) override {
        if (!_net.count(id)) {
            return make_exception_future<raft::read_barrier_reply>(
                std::runtime_error("trying to send a message to an unknown node"));
        }
        if (!(*_connected)(id, _id)) {
            return make_exception_future<raft::read_barrier_reply>(
    ...
    const raft::server_address_set& known_peers() const { return _known_peers; }
    void reset_counters() {
        _servers_added = 0;
        _servers_removed = 0;
    }
    uint32_t servers_added() const { return _servers_added; }
    uint32_t servers_removed() const { return _servers_removed; }
};

template <typename Clock>
typename raft_cluster<Clock>::test_server raft_cluster<Clock>::create_server(size_t id,
                                                                             initial_state state) {
    auto uuid = to_raft_id(id);
    auto sm = std::make_unique<state_machine>(uuid, _apply, _apply_entries, _snapshots.get());
    auto& rsm = *sm;

    std::unique_ptr<raft_cluster::rpc> mrpc = std::make_unique<raft_cluster::rpc>(
        uuid, _connected.get(), _snapshots.get(), _rpc_net, _rpc_config);
    auto& rpc_ref = *mrpc;

    auto mpersistence =
        std::make_unique<persistence>(uuid, state, _snapshots.get(), _persisted_snapshots.get());
    ...
    return {std::move(raft), &rsm, &rpc_ref};
}

template <typename Clock>
raft_cluster<Clock>::raft_cluster(test_case test, apply_fn apply, size_t apply_entries,
                                  size_t first_val, size_t first_leader, bool prevote,
                                  typename Clock::duration tick_delta, rpc_config rpc_config)
...
future<> raft_cluster<Clock>::start_all() {
    co_await coroutine::parallel_for_each(_servers, [](auto& r) { return r.server->start(); });
...
future<> raft_cluster<Clock>::add_entries_concurrent(size_t n, std::optional<size_t> server) {
    ...
    _next_val += n;
    return parallel_for_each(boost::irange(start, _next_val),
                             [this, server](size_t v) { return add_entry(v, server); });
}
...
future<> raft_cluster<Clock>::add_entry(size_t val, std::optional<size_t> server) {
    while (true) {
        try {
            ...
            // FIXME: in some cases when we get `commit_status_unknown` the entry may
            // have been applied. Retrying it could lead to double application which
            // causes hard to debug failures, e.g. #14029. For now we leave a warning
            // so the logs give a hint if such a failure happens and we need to debug
            // it. Ideally we would never have to handle `commit_status_unknown` but
            // some replication tests rely on retrying it during leader changes etc.
            tlogger.warn(
                "replication_test: got `commit_status_unknown` from `add_entry`"
                ", val: {}, server: {}",
                val, server);
        } catch (raft::dropped_entry& e) {
            // retry if an entry is dropped because the leader have changed after it
            // was submitted
        }
    }
}
...
future<> raft_cluster<Clock>::init_raft_tickers() {
    ...
    for (auto s : _in_configuration) {
        _tickers[s].set_callback([&, s] { _servers[s].server->tick(); });
    }
...
void raft_cluster<Clock>::set_ticker_callback(size_t id) noexcept {
    _tickers[id].set_callback([&, id] { _servers[id].server->tick(); });
}
...
future<> raft_cluster<Clock>::elect_new_leader(size_t new_leader) {
    ...
    if (_prevote) {
        ...
        pause_tickers();
        // Leader could be already partially disconnected, save current connectivity
        // state
        ...
        // Make move all nodes past election threshold, also making old leader
        // follower
        elapse_elections();

        do {
            // Consume leader output messages since a stray append might make new
            // leader step down
            co_await yield();  // yield
            _servers[new_leader].server->wait_until_candidate();

            if (both_connected) {
                // Allow old leader to vote for new candidate while not looking alive to
                // others Re-connect old leader
        ...
        *_connected = prev_disconnected;
        ...
        do {
            ...
            pause_tickers();
            // Leader could be already partially disconnected, save current
            // connectivity state
            struct connected prev_disconnected = *_connected;
            // Disconnect current leader from everyone
            _connected->disconnect(to_raft_id(_leader));
...
future<> raft_cluster<Clock>::change_configuration(set_config sc) {
    ...
    BOOST_CHECK_MESSAGE(new_config.contains(_leader) || sc.size() < (_servers.size() / 2 + 1),
                        "New configuration without old leader and below quorum size (no "
                        "election)");
    ...
    // Start nodes in new configuration but not in current configuration
    // (re-added)
...
future<> raft_cluster<Clock>::partition(::partition p) {
    ...
    if (next_leader) {
        ...
        // No leader specified, wait log for all connected servers, before
        // disconnections
...
future<> raft_cluster<Clock>::read(read_value r) {
    ...
    BOOST_CHECK_MESSAGE(val == expected, format("Read on server {} saw the wrong value {} != {}",
                                                r.node_idx, val, expected));
}
...
struct run_test {
    future<> operator()(test_case test, bool prevote, typename Clock::duration tick_delta,
        ...
        raft_cluster<Clock> rafts(test, ::apply_changes, test.total_values, test.get_first_val(),
                                  test.initial_leader, prevote, tick_delta, rpc_config);
        ...
        for (auto update : test.updates) {
            co_await std::visit(
                make_visitor(
                    ...
                    [&rafts](::isolate update) -> future<> { co_await rafts.isolate(update); },
                    [&rafts](partition update) -> future<> { co_await rafts.partition(update); },
                    [&rafts](stop update) -> future<> { co_await rafts.stop(update); },
                    [&rafts](reset update) -> future<> { co_await rafts.reset(update); },
                    [&rafts](wait_log update) -> future<> { co_await rafts.wait_log(update); },
                    ...
                    [&rafts](tick update) -> future<> { co_await rafts.tick(update); },
                    [&rafts](read_value update) -> future<> { co_await rafts.read(update); }),
                std::move(update));
        }
...
template <typename Clock>
void replication_test(struct test_case test, bool prevote, typename Clock::duration tick_delta,
                      rpc_config rpc_config = {}) {
