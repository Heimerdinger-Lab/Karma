...
//     co_context::channel<std::variant<read_barrier_reply, error>, 1> chan;
//     // optimized_optional<abort_source::subscription> abort;
// };

// struct awaited_index {
//     // seastar::promise<> promise;
//     co_context::channel<std::variant<std::monostate, error>, 1> chan;
//     // optimized_optional<abort_source::subscription> abort;
// };
...
//     // co_context::task<add_entry_reply> execute_modify_config(server_id from, std::vector<config_member> add, std::vector<server_id> del) override;
...
//     std::optional<co_context::channel<std::variant<read_barrier_reply, error>, 1>> _stepdown_promise;
//     std::optional<co_context::channel<std::variant<read_barrier_reply, error>, 1>> _leader_promise;
//     std::optional<awaited_conf_change> _non_joint_conf_commit_promise;
//     std::optional<co_context::channel<std::variant<read_barrier_reply, error>, 1>> _state_change_promise;
...
//         co_context::channel<std::variant<std::monostate, error>, 1> done; // notify when done here
...
//         co_context::channel<std::variant<std::monostate, error>, 1> f;
...
//     std::unordered_map<uint64_t, co_context::channel<std::variant<std::monostate, error>, 1>> _aborted_snapshot_transfers;
...
//     std::unordered_map<server_id, co_context::channel<std::variant<snapshot_reply, error>, 1>> _snapshot_application_done;

//     struct append_request_queue {
//         size_t count = 0;
//         co_context::channel<std::monostate, 1> f;
//     };
//     std::map<server_id, append_request_queue> _append_request_status;
...
//     // co_context::task<> drop_waiters(std::optional<index_t> idx = {});
...
//     // }
//     // co_context::task<> do_on_leader_with_retries(AsyncAction&& action);
...
// // co_context::task<> server_impl::wait_for_next_tick(seastar::abort_source* as) {
// //     check_not_aborted();
...
//                     co_context::log::d("[{}] wait_for_entry {}.{}: entry got truncated away, but has the snapshot's term"
...
//                 co_context::log::d("[{}] wait_for_entry {}.{}: entry got truncated away", id(), eid.term, eid.idx);
...
//     co_context::log::d("[{}] waiting for entry {}.{}", id(), eid.term, eid.idx);

//     // This will track the commit/apply status of the entry
//     auto [it, inserted] = container.emplace(eid.idx, op_status{eid.term});
//     if (!inserted) {
...
//         if (term_of_commit_idx > it->second.term) {
//             // prev_wait.done.set_exception(dropped_entry{});
//             co_await it->second.done.release(dropped_entry{});
//             _stats.waiters_awoken++;
//         } else {
//             // prev_wait.done.set_exception(commit_status_unknown{});
//             co_await it->second.done.release(commit_status_unknown{});
//             _stats.waiters_dropped++;
//         }
//         // container.erase(it);
//     }
...
//     // }
//     co_context::log::d("[{}] adding entry after waiting for memory permit", id());
...
//     }
//     co_context::log::d("[{}] adding a forwarded entry from {}", id(), from);
//     try {
...
// //     { aa(leader) } -> std::same_as<future<stop_iteration>>;
// // }
// // co_context::task<> server_impl::do_on_leader_with_retries(AsyncAction&& action) {
...
//     // co_await do_on_leader_with_retries([&](server_id& leader) -> future<stop_iteration> {
...
// // co_context::task<> server_impl::drop_waiters(std::optional<index_t> idx) {
...
//             co_await _snapshot_application_done[id].release(std::move(m));
...
//                 // load it.
//                 if (!is_local) {
//                     _apply_entries.push(std::move(snp));
//                 }
//             }
