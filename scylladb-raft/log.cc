...
index_t log::maybe_append(std::vector<log_entry_ptr>&& entries) {
    ...
    for (auto& e : entries) {
        if (e->idx <= last_idx()) {
            if (e->idx < _first_idx) {
                co_context::log::d("append_entries: skipping entry with idx {} less than log start {}",
                    e->idx, _first_idx);
                continue;
            }
            if (e->term == get_entry(e->idx)->term) {
                co_context::log::d("append_entries: entries with index {} has matching terms {}", e->idx, e->term);
                continue;
            }
            co_context::log::d("append_entries: entries with index {} has non matching terms e.term={}, _log[i].term = {}",
