...
public:
    ...
    Index commit_idx() {
        co_context::log::d("{}: check committed count {} cluster size {}", std::is_same_v<Index, index_t> ? "commit" : "read barrier", _count, _match.size());
...
void votes::register_vote(server_id from, bool granted) {
    ...
    if (!registered) {
        co_context::log::d("Got a vote from unregistered server {} during election", from);
    }
}
