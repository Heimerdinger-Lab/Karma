...
transport::frame::frame(karma_rpc::OperationCode code) 
    : m_operation_code(code) {
    m_seq = g_frame_id++;
...
void transport::frame::flag_response() {
    m_flag = 1;
}

void transport::frame::flag_request() {
    m_flag = 0;
}

bool transport::frame::is_response() {
    return m_flag == 1;
}

bool transport::frame::is_request() {
    return m_flag == 0;
}

void transport::frame::set_header(std::string header) {
    m_header = header;
}

void transport::frame::set_payload(std::string payload) {
    m_data = payload;
}
...
- crc32 of payload: u32
    */
    ...
    std::cout << "encode frame length = " << frame_length << ", header_length: " << extend_header_length << ", payload: " << payload_length << std::endl;
    ...
    encode_str.insert(encode_str.end(), (char *) &extend_header_length, (char *) &extend_header_length + 4);
...
std::shared_ptr<transport::frame> transport::frame::parse(std::span<char> src) {
    // frame ret{};
    auto ret = std::make_shared<transport::frame>();
    uint32_t offset = 0;
    ...
    ret->m_header.insert(ret->m_header.end(), src.data() + offset, src.data() + offset + header_length);
